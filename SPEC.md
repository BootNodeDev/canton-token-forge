...
checked against the tree at commit `f958d3b`.
...
All three suites were re-run at commit `f958d3b`, exit 0:

| Suite | Result | Needs |
|---|---|---|
| Daml Script | **60 scenarios**, 11 modules | nothing, runs in-process |
| Registry unit | **163 tests**, 10 files | nothing, in-process server with a stub ledger |
...
786 lines of production Daml, 1665 lines of Daml tests, 1479 lines of TypeScript
service, 3474 lines of TypeScript tests. MIT licensed. Pre-release: the package
...
cd registry && npm install && npm test   # 163 unit tests, no ledger needed
