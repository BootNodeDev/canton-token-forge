...
checked against the tree at commit `f510fbb`.
...
All three suites were re-run at commit `f510fbb`, exit 0:

| Suite | Result | Needs |
|---|---|---|
| Daml Script | **60 scenarios**, 11 modules | nothing, runs in-process |
| Registry unit | **195 tests**, 10 files | nothing, in-process server with a stub ledger |
...
service, 4130 lines of TypeScript tests. MIT licensed. Pre-release: the package
...
cd registry && npm install && npm test   # 195 unit tests, no ledger needed
