...
1. Rehearse: run the workflow from the Actions tab. A `workflow_dispatch` run
   performs the build, the suite, the reproducibility check and the smoke
   test, and stops short of publishing, because the publish step is gated on a
   tag push.

   Three things a dispatch leaves unexercised: the refusal of a tag `main` does
   not reach and the publish itself are both gated on that same push, and the
   run sets `ALLOW_UNTAGGED=1`, which switches off `release-notes.sh`'s own
   check that the tag names the checked-out commit. To rehearse all three, push
   a hyphenated tag such as `v0.1.0-rc1` first:
