...
export function allocationRouter(deps: ServerDeps): Router {
  ...
  r.post("/registry/allocation-instruction/v1/allocation-factory", async (req, res) => {
  ...
  for (const choice of ["execute-transfer", "withdraw", "cancel"]) {
    r.post(`/registry/allocations/v1/:allocationId/choice-contexts/${choice}`, async (req, res) => {
