...
# the DAR of every version ever built in dist, so a glob would keep uploading an
# older one after a version bump.
VERSION="$(awk '/^version:/ {print $2; exit}' "$REPO_ROOT/daml/canton-token-forge/daml.yaml")"
if [ -z "$VERSION" ]; then
